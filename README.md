...
**UI v3 (2026-07): bento redesign + new pages + full admin CRUD + test infra**, hand-ported from the Lovable-generated reference in [`ref/make-joyful-moments-main/`](ref/make-joyful-moments-main/) — Sora/Manrope fonts (JetBrains Mono kept for eyebrows/badges/prices), 20px radius, shadcn/ui + Radix component library (`Sheet`/`Dialog`/`Accordion`/`Carousel`/`Select`/`DropdownMenu`/`Table`, `react-hook-form`+`zod` for the review form; admin CRUD forms use plain `useActionState` + server actions with native validation). New public pages: `/about`, `/cities`, `/for-owners`, `/privacy-policy`, `/terms`, plus styled `error.tsx`/`global-error.tsx` boundaries. New **admin CRUD**: sidebar shell replacing the old flat pill nav, full list/detail create-edit-archive flows for Listings, Cities, Areas, Amenities and Owners, with `admin_audit_log` now actually written to (`src/lib/audit.ts`). Regenerated the full icon set from a documented icon-only mark (`scripts/make-app-icons.js`), including correctly-padded maskable PWA icons (a real latent bug this fixed). Listing imagery can now be ingested from Wikimedia Commons URLs (`scripts/scrape-osm.js`) with local branded placeholders as fallback. SEO/AEO/GEO hardened toward Lighthouse SEO 100 (`src/lib/seo.ts`'s `resolveSeo()`, root canonical, expanded `/llms.txt`). **New test infrastructure** (none existed before): Vitest unit tests (`src/lib/__tests__/`) and Playwright E2E specs (`e2e/`) covering the public site, PWA, SEO/a11y (axe-core), and admin CRUD — mutating specs (`add-your-pg`, `contact-reveal`, admin writes) are gated behind `E2E_ALLOW_WRITES=1` since there's no separate staging Supabase project yet. See [DESIGN_SYSTEM.md → Bento theme](docs/DESIGN_SYSTEM.md#bento-theme-implemented-2026-07--current-source-of-truth) and [ADMIN_PANEL_SPEC.md](docs/ADMIN_PANEL_SPEC.md).

- Schema live on Supabase — [`supabase/migrations/`](supabase/migrations/) (all tables from [DATABASE_SCHEMA.md](docs/DATABASE_SCHEMA.md), RLS, triggers; `0002` adds the public `listing-images` Storage bucket + nullable `pg_type`; `0004` adds `cities.tagline`/`hero_image_url` for the Cities directory, additive-only)
...
- Routes: `/` (hero, search, map, explore citywise, story), `/pg/[city]` (+ `?type=` filter), `/pg/[city]/[area]/[slug]` (detail + contact-reveal lead capture), `/add-your-pg` (owner submission with photo upload to Supabase Storage → `pending_review`), `/about`, `/cities`, `/for-owners`, `/privacy-policy`, `/terms`, `sitemap.xml`, `robots.txt`, `manifest`, JSON-LD
...
- Tests: `npm run test` (Vitest, unit-level, no live DB needed) and `npm run test:e2e` (Playwright — auto-builds and boots a prod server on port 3100 via `playwright.config.ts`'s `webServer`, or point `PLAYWRIGHT_BASE_URL` at an already-running server; run `npx playwright install --with-deps chromium` once first). Specs that write real rows (`add-your-pg`, `contact-reveal`, admin create/edit/archive) are skipped unless `E2E_ALLOW_WRITES=1` is set — there's no separate staging Supabase project today, so these hit the real DB and are off by default; test rows are prefixed `e2e-test-` for manual cleanup
