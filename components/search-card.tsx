...
export function SearchCard({ cities }: { cities: CityOption[] }) {
  ...
  const matches = useMemo(() => {
    ...
    return cities.filter((c) => c.name.toLowerCase().includes(t) || c.state.toLowerCase().includes(t)).slice(0, 8);
  }, [q, cities, launched]);

  function go(slug: string) {
    setOpen(false);
    trackEvent("search_performed", { city: slug, filters_applied: [type, budget].filter(Boolean).join(",") || "none" });
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-grey-50 bg-white p-6 shadow-[0_30px_60px_-24px_rgba(83,74,183,0.28)]">
      <span className="mb-3.5 block font-mono text-[11px] font-semibold tracking-wider text-primary">FIND A PLACE →</span>
      ...
      <div className="relative mb-3.5">
        <label htmlFor="hero-city" className="mb-1.5 block text-[12.5px] font-semibold text-grey-600">
          City
        </label>
        <input
          id="hero-city"
      ...
      <div className="mb-4">
        <label htmlFor="hero-budget" className="mb-1.5 block text-[12.5px] font-semibold text-grey-600">
          Budget (per month)
        </label>
        <select id="hero-budget" value={budget} onChange={(e) => setBudget(e.target.value)} className={fieldCls}>
          {BUDGETS.map((b) => (
      ...
      <button type="submit" className="w-full rounded-md bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-primary-dark">
        Search listings
      </button>
