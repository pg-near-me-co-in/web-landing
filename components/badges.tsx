...
const BADGE_BASE = "inline-flex items-center gap-1 rounded-full px-2.5 py-1 font-mono text-[11.5px] font-semibold";
...
export function PgTypeBadge({ type }: { type: PgType | null }) {
  if (!type) return null;
  return <span className={`${BADGE_BASE} ${PG_TYPE_STYLE[type]}`}>{PG_TYPE_LABEL[type]}</span>;
}

export function VerifiedBadge() {
  return <span className={`${BADGE_BASE} bg-success-bg text-teal-dark`}>✓ Verified</span>;
}
...
export function RatingStars({ rating, count }: { rating: number | null; count?: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-sm font-semibold text-grey-700">
      ...
      {count != null && count > 0 && <span className="font-normal text-grey-500">({count})</span>}
    </span>
  );
}
